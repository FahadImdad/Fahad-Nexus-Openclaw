      ...
      <div className="snd-wrap">
        {sndHint && !userMuted && (
          <motion.div className="snd-hint" initial={{ opacity: 0, x: 12 }}
            ...
            sound on · tap to mute <span className="snd-arrow">➜</span>
          </motion.div>
        )}
        <motion.button type="button" className={`snd-toggle ${userMuted ? "off" : ""}`} onClick={toggleMute}
          whileTap={{ scale: 0.85 }} aria-label={userMuted ? "Turn sound on" : "Turn sound off"}
          title={userMuted ? "Sound off · tap to unmute" : "Sound on · tap to mute"}>
          {sndHint && !userMuted && <span className="snd-ping" />}
          {userMuted ? (
