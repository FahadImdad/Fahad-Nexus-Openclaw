...
const chat = [
  ...
  { who: "user", text: "John Carter, 12 Lake Street, Chicago." },
...
function LiveChat() {
  ...
  const toggleMute = () => {
    ...
    if (!a) return;
    if (userMutedRef.current) {                     // OFF -> ON
      userMutedRef.current = false; setUserMuted(false);
      a.volume = 1;
      if (soundOnRef.current) a.muted = false;      // unmute right where it is
      else enableSound().catch(() => {});           // this tap is the gesture browsers need
      return;
    }
    if (!soundOnRef.current) {                      // browser hasn't granted sound yet:
      enableSound().catch(() => {});                // this tap turns it on, not off
      return;
    }
    userMutedRef.current = true; setUserMuted(true); // ON -> OFF: keep running, silent
    a.muted = true;
  };
  ...
  const onAudioTime = () => {
    const a = audioRef.current;
    if (!a || a.paused || hungUp.current) return;  // never let a stray tick undo a hang-up
      ...
      <div className="snd-wrap">
        {sndHint && !userMuted && (
          <motion.div className="snd-hint" initial={{ opacity: 0, x: 12 }}
            ...
            {soundOn ? "sound on · tap to mute" : "tap for sound"} <span className="snd-arrow">➜</span>
          </motion.div>
        )}
        <motion.button type="button" className={`snd-toggle ${userMuted ? "off" : ""}`} onClick={toggleMute}
          whileTap={{ scale: 0.85 }} aria-label={userMuted || !soundOn ? "Turn sound on" : "Turn sound off"}
          title={userMuted ? "Sound off · tap to unmute" : soundOn ? "Sound on · tap to mute" : "Tap for sound"}>
          {!userMuted && (sndHint || !soundOn) && <span className="snd-ping" />}
          {userMuted || !soundOn ? (
            <svg width="15" height="15" viewBox="0 0 24 24" fill="#fff"><path d="M16.5 12c0-1.77-1-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3 3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4 9.91 6.09 12 8.18V4z"/></svg>
          ) : (
            <svg width="15" height="15" viewBox="0 0 24 24" fill="#fff"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1-3.29-2.5-4.03v8.05c1.5-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z"/></svg>
          )}
        </motion.button>
      </div>
      ...
      {phase === "ring" && (
        <motion.div className="ic-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          <div className="ic-actions">
            ...
            <div className="ic-a">
              <div className={`ic-btn accept ${accepted ? "pressed" : ""}`} onClick={acceptCall} style={{ cursor: "pointer" }} title="Answer now">
